          ...
          {orders.length === 0 ? <p className="dashboard-empty">No hay pedidos pagados pendientes de preparación.</p> : (
            <div className="dashboard-table-wrap">
              <table className="dashboard-table">
                <thead><tr><th>Pedido</th><th>Cliente</th><th>Productos y receta</th><th>Estado</th><th>Insumos</th><th>Acción</th></tr></thead>
                <tbody>{orders.map((order) => {
                  ...
                  return <tr key={order.id}>
                    ...
                    <td><button type="button" disabled={loadingId === order.id} onClick={() => updateStatus(order)}>{loadingId === order.id ? 'Actualizando...' : (order.status === 'pending' ? 'Iniciar preparación' : 'Marcar listo')}</button></td>
                  </tr>;
                })}</tbody>
              </table>
            </div>
